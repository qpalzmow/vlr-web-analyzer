...
_in_flight: dict[tuple[str, str], Future] = {}
...
def get_cached_data(cache_type: str, key: str, fetch_func, *args, **kwargs):
    ...
    if not is_leader:
        # Follower waits for the in-flight leader's Future result
    ...
    try:
        ...
        logger.error("Error fetching %s for key %s: %s", cache_type, key, e)
        raise
    finally:
        with _cache_lock:
